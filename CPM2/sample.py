...
class Sample:
    ...
    def do_step(self):
        self.cpm.I,self.cpm.A,self.cpm.P = do_step(self.cpm.I, self.cpm.num_x, self.cpm.num_y, self.zmasks.z_masks, self.zmasks.dP_z, self.cpm.A, self.cpm.P, self.cpm.lambda_A, self.cpm.lambda_P, self.cpm.A0, self.cpm.P0, self.cpm.J_diff, self.T,self.zmasks.primes,self.zmasks.hashes)

    def do_steps(self):
        self.cpm.I,self.cpm.A,self.cpm.P = do_steps(self.n_steps,self.cpm.I, self.cpm.num_x, self.cpm.num_y, self.zmasks.z_masks, self.zmasks.dP_z, self.cpm.A, self.cpm.P, self.cpm.lambda_A, self.cpm.lambda_P, self.cpm.A0, self.cpm.P0, self.cpm.J_diff, self.T,self.zmasks.primes,self.zmasks.hashes)


@jit(nopython=True)
def do_step(I,num_x,num_y,z_masks,dP_z,A,P,lambda_A,lambda_P,A0,P0,J_diff,T,primes,hashes):
...
@jit(nopython=True)
def do_steps(n_steps,I,num_x,num_y,z_masks,dP_z,A,P,lambda_A,lambda_P,A0,P0,J_diff,T,primes,hashes):
    for i in range(n_steps):
        I,A,P = do_step(I,num_x,num_y,z_masks,dP_z,A,P,lambda_A,lambda_P,A0,P0,J_diff,T,primes,hashes)
    return I,A,P
...
@jit(nopython=True)
def pick_pixel(I,nx,ny):
    picked = False
    while picked is False:
        i = int(np.random.random()*nx)
        j = int(np.random.random()*ny)
