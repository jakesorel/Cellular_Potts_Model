...
if __name__ == "__main__":
    ...
    if not os.path.exists("results/compiled/variable_soft"):
        os.mkdir("results/compiled/variable_soft")
    for i, lpm in enumerate(lambda_P_mult_range):
        dir_name = "results/compiled/variable_soft/%.2f"%lpm
        if not os.path.exists(dir_name):
            os.mkdir(dir_name)

    for lpm in lambda_P_mult_range:

        I_save_sparse = cPickle.load(bz2.BZ2File("results/variable_soft/%.2f/%d.pbz2"%(lpm,iter_i), 'rb'))
        ...
        df.to_csv("results/compiled/variable_soft/%.2f/%d.csv"%(lpm,iter_i))
