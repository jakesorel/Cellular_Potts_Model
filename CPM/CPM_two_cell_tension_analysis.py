...
num_clusters = np.array(out).reshape(BB.shape[0],BB.shape[1],Js.size,3)
...
ax.tricontourf(BB.ravel(),TT.ravel(),cluster_index.ravel(),levels=levels)
ax.set(yscale="log")
fig.show()
...
nfine = 200
beta_spacefine, logT_spacefine = np.linspace(0,1, nfine), np.linspace(0, 2, nfine)
