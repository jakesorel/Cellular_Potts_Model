...
def get_polarity_cluster_repeat(i,j):
    I_save = load_npz("CPM/dynamic_jamming/%d_%d_1.npz" % (i, j)).toarray()
    I_save = I_save.reshape((n_save, cpm.num_x, cpm.num_y))
    cpm.I_save = I_save
    cpm.get_centroids_t()
    pol = np.array([polarity_centroids(centroids, ES_ids) for centroids in cpm.centroids])
...
lazy_results = []
for inputt in inputs:
    lazy_result = dask.delayed(get_polarity_cluster_repeat)(*inputt)
    lazy_results.append(lazy_result)
out2 = dask.compute(*lazy_results)


APt = np.array(out).reshape(15,15,200)
APt2 = np.array(out2).reshape(15,15,200)
APt_all = np.zeros((15,30,200))
APt_all[:,:15] = APt
APt_all[:,15:] = APt2
APt = APt_all
...
fig.subplots_adjust(top=0.9, bottom=0.15, left=0.25, right=0.75,wspace=0.05)
fig.savefig("jamming diff time.pdf",dpi=300)
