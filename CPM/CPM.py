...
class CPM:
    ...
    def update_tension(self,t):
        beta = self.dynamic_var(t,self.t0,self.beta_start,self.beta_end,self.tau)
        ...
        lambda_A, lambda_P, W, P0, A0 = get_normal_params(p0=8, r=100, beta=beta, gamma=0, delta=0.7, epsilon=0.8, A0=30,eta = self.eta)
        self.make_J(W)
    ...
    def run_simulation_dynamictension(self,n_steps,n_save,polarise=False):
        I = self.I
        i_save = np.zeros(n_steps).astype(bool)
        i_save[::int(n_steps/n_save)] = True
        n_save = i_save[::int(n_steps/n_save)].size
