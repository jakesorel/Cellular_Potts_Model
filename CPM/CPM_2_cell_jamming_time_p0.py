...
def do_job(inputt):
    ...
    cpm.t0 = t0
    cpm.eta = 1
    cpm.p0start,cpm.p0end = 8,4
    # cpm.lPstart = 1
    # cpm.lPend = 0.6
    lambda_A, lambda_P, W_start, P0, A0 = get_normal_params(p0=8, r=100, beta=0.4, gamma=0, delta=0.7, epsilon=0.8, A0=30,
                                                      eta=cpm.eta)
    ...
    save_npz("dynamic_tension_p0/%d_%d.npz"%(int(sys.argv[1]),int(Id)), I_SAVE)


if __name__ == "__main__":
    if not os.path.exists("dynamic_tension_p0"):
        os.makedirs("dynamic_tension_p0")
