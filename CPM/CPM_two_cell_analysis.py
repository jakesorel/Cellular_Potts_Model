...
def get_num_clusters(i,j):
    I_save = load_npz("CPM/two_cell_results_lP_T/%d_%d.npz" % (i, j)).toarray()
...
Is,Js = np.arange(int(15**2)),np.arange(15)
...
nfine = 200
lPmult_spacefine, logT_spacefine = np.linspace(0.5, 5, nfine), np.linspace(0, 2, nfine)
LLf,lTTf = np.meshgrid(lPmult_spacefine,logT_spacefine,indexing="ij")


z = bisplev(lPmult_spacefine,logT_spacefine, bisplrep(LL.ravel(),np.log10(TT).ravel(),cluster_index.ravel(),s=2))
fig, ax = pl
...
extent = [lPmult_space.min()*0.3,lPmult_space.max()*0.3,logT_spacefine.min(),logT_spacefine.max()]
...
# fig.show()
fig.savefig("lambda P T phase diagram smooth.pdf",dpi=300)
