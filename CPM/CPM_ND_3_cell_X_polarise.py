...
def get_polmag_image(self,s):
    self = cpm
    s = self.I.max() - 3
    cell_edge = self.get_perimeter_elements_Moore(self.I == s) * (self.I == s)
    I,J = np.nonzero(cell_edge)
    out = np.zeros_like(self.I).astype(np.float64)
    dthetas=[]
    for i,j in np.array([I,J]).T:
        ...
        # # if prop> 0.5:
        # #     prop = 0.5
        # # polmag = 0.5 - prop*np.cos(dtheta)
        # polmag = 0.0 + (np.pi - dtheta < prop * np.pi) * 1.0
        # # polmag = dtheta
        # # print(polmag)
        ...
        direc /= np.linalg.norm(direc)
        prop = self.cells[s].prop
        dtheta = np.arccos(np.dot(direc, direc0) - 1e-17)
        # if prop> 0.5:
        #     prop = 0.5
        # polmag = 0.5 - prop*np.cos(dtheta)
        polmag = self.pol_amount + (np.pi - dtheta > prop * np.pi) * (1.0 - self.pol_amount)
        print(direc)
        out[i,j] = polmag
    plt.imshow(out + 0.2*(self.I!=0))
    plt.scatter(bound_centroid[0],bound_centroid[1],color="white")
    plt.show()


def get_polarity_vector(self,s):
    ...
    if prop!=0:
        ...
        centroid = get_centroid(cell_edge)
        edge_centroid = get_centroid(cell_bound)
        direc = edge_centroid-centroid
        direc /= np.linalg.norm(direc)
        theta0 = np.arccos(direc[0])
        # 0.5 - prop*np.cos(theta - theta0)
